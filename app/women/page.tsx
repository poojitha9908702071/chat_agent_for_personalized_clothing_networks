...
export default function WomenPage() {
  ...
  useEffect(() => {
    const loadProducts = async () => {
      setLoading(true);
      try {
        console.log("Fetching women's products from database...");
        
        // Fetch women's products by category from database
        const categoryPromises = [
          getProductsByCategory("Women's Bottomwear", "Women", 50),
          getProductsByCategory("Women's Topwear", "Women", 50),
          getProductsByCategory("Women's Dresses", "Women", 50),
          getProductsByCategory("Women's Outerwear", "Women", 50),
          searchProducts("women dress", "fashion"),
          searchProducts("women clothing", "fashion"),
          searchProducts("women fashion", "fashion")
        ];
        ...
        results.forEach((products, index) => {
          console.log(`Category ${index} returned ${products.length} products`);
        ...
        console.log(`Total unique products from database: ${uniqueProducts.length}`);
        
        // Filter for women's products
        const womenProducts = uniqueProducts.filter(p => {
          ...
          if (category.includes('women')) return true;
        ...
        console.log(`Women's products: ${womenProducts.length} out of ${uniqueProducts.length}`);
        
        // Add fallback products if we have few products
        ...
        if (womenProducts.length < 10) {
          console.log("Adding fallback products for better variety");
        ...
        console.log("API error occurred, using fallback products");
        const fallbackProducts = [
          {
            id: 1,
            product_id: "W001",
