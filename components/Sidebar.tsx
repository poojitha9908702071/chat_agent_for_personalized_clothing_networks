...
const items = [
  ...
  { 
    key: "Kids", 
    icon: "👧",
    categories: ["Boys Clothing", "Girls Clothing", "Baby Clothing"],
    link: "/kids"
  },
];
    ...
    <>
      ...
      <aside className={`fixed left-0 top-0 h-full w-80 bg-gradient-to-b from-pink-50 to-white shadow-2xl z-50 transform transition-transform duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-2xl font-bold bg-gradient-to-r from-pink-600 to-pink-800 bg-clip-text text-transparent">Menu</h3>
          <button
            onClick={() => setIsOpen(false)}
        ...
        <div className="mb-6">
          <h4 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-3 px-3">Special Features</h4>
          <div className="space-y-3">
            {specialFeatures.map((feature) => (
              <button
                ...
                className={`w-full group relative overflow-hidden rounded-xl p-4 bg-gradient-to-r ${feature.gradient} text-white shadow-lg hover:shadow-2xl transition-all transform hover:scale-105`}
              >
                <div className="absolute inset-0 bg-white/10 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                <div className="relative flex items-center gap-3">
                  <span className="text-3xl">{feature.icon}</span>
                  <div className="text-left flex-1">
                    <div className="font-bold text-sm">{feature.key}</div>
                    <div className="text-xs opacity-90">{feature.description}</div>
                  </div>
                  <span className="text-xl opacity-75 group-hover:opacity-100 group-hover:translate-x-1 transition-all">→</span>
                </div>
              </button>
            ))}
          </div>
