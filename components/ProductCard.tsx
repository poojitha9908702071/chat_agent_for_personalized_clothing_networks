...
export default function ProductCard({ productId, title, price, img, onAdd, onBuy, isWishlisted, onToggleWishlist, qty = 0, onIncrease, onDecrease, onClick }: Props) {
  const handleClick = () => {
    if (onClick) {
      onClick();
    }
  ...
  return (
    <div className="group relative rounded-lg border border-gray-200 bg-white p-4 hover:shadow-lg transition-shadow duration-200 flex flex-col h-full">
      ...
      <div className="text-lg font-semibold text-black mb-3">₹{price ?? "-"}</div>
